...
class ArticleController extends FOSRestController
{
     ...
     * @ParamConverter("articleNew", converter="fos_rest.request_body")
     */
    public function updateAction(Article $article, Article $articleNew, ConstraintViolationList $violations)
    {
        if (count($violations)) {
            $message = 'The JSON sent contains invalid data. Here are the errors you need to correct: ';
            foreach ($violations as $violation) {
    ...
    public function removeAction(Article $article)
    {
        ...
        // Si l'article existe bien (évite l'erreur de supprimer un article inexistant)
        if ($article) {
            $em->remove($article);
            $em->flush();
        }
